...
class TestToken(TestCase):
    ...
    def test_init_no_token_given(self):
        ...
        self.assertEqual(
            t.payload[api_settings.TOKEN_TYPE_CLAIM], MyToken.token_type)
    ...
    def test_init_token_given(self):
        ...
        self.assertEqual(t["exp"], datetime_to_epoch(
            original_now + MyToken.lifetime))
    ...
    def test_init_bad_sig_token_given(self):
        ...
        token_1 = jwt.encode(
            payload, api_settings.SIGNING_KEY, algorithm="HS256")
        payload["foo"] = "baz"
        token_2 = jwt.encode(
            payload, api_settings.SIGNING_KEY, algorithm="HS256")
    ...
    def test_set_exp(self):
        ...
        token.set_exp()
        self.assertEqual(token["exp"], datetime_to_epoch(
            now + MyToken.lifetime))

        # Should allow overriding of beginning time, lifetime, and claim name
        token.set_exp(claim="refresh_exp", from_time=now,
                      lifetime=timedelta(days=1))
